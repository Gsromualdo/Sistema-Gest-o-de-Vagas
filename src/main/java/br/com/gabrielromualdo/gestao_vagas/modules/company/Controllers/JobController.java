...
import br.com.gabrielromualdo.gestao_vagas.modules.company.UseCases.CreateJobUseCase;
import br.com.gabrielromualdo.gestao_vagas.modules.company.UseCases.ListAllJobsByCompanyUseCase;
...
public class JobController {
    ...
    @GetMapping("/")
    @PreAuthorize("hasRole('COMPANY')")
    @Tag(name = "Vagas", description = "Listagem de das vagas")
    @Operation(summary = "Cadastro de vagas", description = "Essa função é responsável por listar as vagas da empresa")
    @ApiResponse(responseCode = "200", description = "Sucesso", content = {
            @Content(schema = @Schema(implementation = JobEntity.class))
    })
